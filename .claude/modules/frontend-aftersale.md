...
> ## 📌 相关专项：售后工单 AI 预标注（五轮回测，2026-07 完成）
>
> **总结存档：`handoff/2026-07-30-aftersale-ai-preannotation-SUMMARY.md`**
> ——问到这件事的任何细节先读它，不要凭记忆报数字（跨 18 份 handoff，已汇总成一份）。
>
> 速查：
> - 最可靠成果是**高置信批次**（四轮准确率 100/99.3/99.0/95.3，约占 20%），可作快速审核
>   通道；但呈下滑趋势、非恒定 99%+，**不建议自动写回**。
> - **型号匹配必须用确定性代码**：LLM 逐单推理会塌缩（系列层 25.7%），规则代码五个月稳定
>   91.8~94.3%。评估**以系列层为准**（人工在型号层的标注本身含噪声）。
> - 三个可复用组件在 `backend/scripts/aftersale/`：`model_matcher.py`（型号）、
>   `ignore_rule.py`（忽略判定机械部分）、`material_hint.py`（发货物料材质→实木/钣金）。
> - **待办**：用户要改原因词典三处（「安装痕迹」归属、缺「学习灯控制失效」、
>   「升降托盘失效」应为「旋转托盘破损」），**改完必须重新导出字典再跑下一轮**。
> - 回测数据在 `tmp/holdout-*` 与 `tmp/backtest-202602/`（已 gitignore，含客户备注）。
>
> 全程只读、未写回任何生产数据。
...
- 待处理列表按发货日期**降序**排列（最新在前）
- 确认 → `POST /api/aftersale/cases`；忽略 → `POST /api/aftersale/cases/:order_no/ignore`
- 需要 `aftersale:edit` 权限才显示确认/忽略/添加原因按钮
- **多内容独立备注**：售后内容条数 ≥ 2 时，每条内容卡片显示独立「商家备注」textarea；提交时各条 reason 携带自己的 `seller_remark`，后端按备注文本分组调用关键词学习，避免不同原因互相污染候选词池；单条内容时不显示独立备注输入框，使用工单级备注
...
- `AftersaleCasesTable.vue` 展开行：`loadData()` Phase 1 后追加 `POST /api/aftersale/cases/media-flags` 批量查当前页 has_media 计数（不逐行查）；展开时才 `GET /cases/<order_no>/media` 懒加载详情；点击卡片打开 `src/components/common/MediaViewer.vue`（通用查看器，2026-07-28 起从 `AftersaleMediaViewer.vue` 重构为通用组件，供其它模块复用），全屏+图片缩放平移+左右切换；删除通过 `:delete-handler` 回调 prop 传入（不再是组件内硬编码的售后接口），本页传 `canEditAftersale ? deleteAftersaleMedia : null`；筛选变化时清空媒体缓存（`watch(props.filter)`）
