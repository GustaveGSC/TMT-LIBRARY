...
## P3 · 停用状态改为「ERP 默认 + 人工例外覆盖」（用户已确认采纳）

`product_material.is_disabled` 原本是**纯人工维护**，8,086 条物料靠手工标停用不现实。
用户已同意改为默认取 ERP、人工只做例外覆盖——与大类判定同一思路。

### 用户追问：`状态=失效` 与品名含「已停用」是否吻合？→ **实测严重不吻合**

对真实导出 8,086 行做交叉统计：

| 组合 | 条数 | 占比 |
|---|---|---|
| 状态失效 + 名字含「已停用」 | 204 | 2.5% |
| 状态生效 + 名字不含 | 5,167 | 63.9% |
| **状态生效 + 名字含「已停用」** | **2,654** | **32.8%** |
| 状态失效 + 名字不含 | 61 | 0.8% |
| **合计不一致** | **2,715** | **33.6%** |

- 名字标了停用的 2,858 条里，**只有 204 条（7.1%）在 ERP 状态上是失效的**；
- 265 条失效里有 61 条（23%）名字没标；
- 不一致几乎是**单向**的（各分组都是「名字标了但状态生效」）；
  反方向的 61 条里有 40 条集中在 `03 金蝶原材料物料`。

**结论：两个信号基本独立，且名称标记是完整得多的那个（2,897 vs 265）——
ERP 的 `状态` 字段实际上几乎没有被维护。**
任何一个单独用都不够：只用 `状态`，2,654 条明显已废弃的物料仍会出现在选择候选里；
只用名称，会漏掉 61 条 ERP 已置失效的。

**并集（任一为停用）= 2,919 条 = 36.1%。**

### 关键：名称关键词**必须做成配置**，不得写死在代码里
...
有 **39 条**用的是「停用」而非「已停用」，所以只匹配「已停用」会漏。
但更重要的是——**用户已明确要求把停用关键词做成配置项**：

> 「你提出这几个物料都明确是 停用 的物料，可以把停用的关键字也作为一个配置」

这与大类归属同一个原则：**判定依据由用户在界面上配，代码里不写死常量。**
录入人员将来可能用新的写法（「淘汰」「废弃」「不再使用」…），
写死在代码里就意味着每次都要改代码 + 重新部署。

新建一张关键词表（参考 `aftersale_shipping_ignore_term` 的既有形态，保持仓库风格一致）：

```
material_disable_keyword
  id          Integer PK
  keyword     String(64)  NOT NULL UNIQUE
  is_disabled Boolean     NOT NULL default False   # 规则本身的启用开关
  remark      String(255) NULL
  created_at  DateTime    NOT NULL
```

要点：
- **迁移里可以预置 `停用`、`作废` 两条种子**——这两个是实测有命中的既有写法，
  预置是给用户一个可用起点，与「不预置分组大类」不冲突
  （分组大类是 67 个业务判断，这里是 2 个已被数据证实的既有事实）。
  但请在迁移注释里写清预置了什么，我部署时会核对。
- 匹配对象是 `import_product_raw.name`，**子串匹配**即可（中文无大小写问题）；
  匹配前对 name 做 `strip()`。
- 关键词表与 `erp_code_rules` 一样要**纳入内存缓存并在增删改后失效**，
  不要每次列表请求都查表。
- 关键词为空表（用户全删了）时，默认停用判定只剩 `status == '失效'`，不要报错。

### 前端（我负责）

`编码规则` tab 下再加一个子 tab「停用规则」，包含：
- ERP `状态` 的说明（只读，说明它来自 ERP 且实测维护率很低）；
- 关键词列表的增删改与启用开关；
- 一个实时的命中条数预览，让用户改关键词时能立刻看到影响多少条物料
  —— 这个需要一个轻量接口，建议 `GET /api/material/disable-preview`
  返回 `{status_inactive: N, keyword_hit: N, union: N}`，一次聚合查询即可，
  **不要按关键词逐个查**。

### 需要的改动

1. `import_product_raw` 新增 `status` 列，导入 `状态`（表头别名：`('状态',)`）；
2. 默认停用判定 = `status == '失效'` **或** `name` 含**任一启用的配置关键词**，
   与大类判定一样**在查询时实时计算**，不落库；
3. **`product_material.is_disabled` 当前的类型表达不了这个设计** —— 它是
   `Boolean, nullable=False, default=False`，只有真/假两态，无法表达「跟随 ERP」。
   需要改成**三态**：`NULL` = 跟随 ERP 默认、`True` = 人工强制停用、`False` = 人工强制启用。
   即把该列改为 `nullable=True` 且**去掉 server_default**。
   **这张表目前是空的，现在改代价为零**，等有数据后再改就要迁移 + 回填。
   接口返回建议同时给 `is_disabled`（最终生效值）与 `is_disabled_override`（人工设定值，可为 null），
   前端才能区分「默认停用」和「人工停用」——这部分前端我会配合调整。
4. 请注意 36.1% 默认停用是个很大的比例，物料清单的「显示已停用」开关会成为常用功能。
