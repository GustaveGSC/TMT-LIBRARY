...
用户已把 `01 · 金蝶成品物料`（167 条）与 `03 · 金蝶原材料物料`（790 条）
的分组默认大类改为**仅「无用物料」**（我已用 `save_group` 服务改好，
`updated_by=claude`）。这两组是启用现行编码系统之前的金蝶旧编码物料，
大部分已停用不再使用。

顺带效果：「无用物料」大类 963 条，**未分类从 3,348 降到 2,391**。

## 要改的

`POST /api/material/items/<code>/prices`（`material_price_service.add_price`）
在建价格前判断：**该物料的大类若命中 `useless`，直接拒绝**。

```
若 'useless' in material['categories'] → Result.fail('无用物料不支持维护价格')
```

要点：

1. **必须在惰性创建 `cost_bom_node` 之前拦**，否则会给无用物料留下一个
   空的成本节点（脏数据，且它会占用 `code` 的 UNIQUE 名额，
   影响同 base 码的正常物料）。这一条请写测试。
2. `categories` 是**多标签**。若某物料同时是 `useless` 和别的大类，
   我的判断是**只要命中 useless 就拒绝**（用户语义是"这一组不要了"）。
   如果你认为该改成"仅当只有 useless 时才拒绝"，先说，别直接改。
   > 实际数据里 01/03 两组都只勾了 useless，前缀例外规则对它们是 0 条，
   > 所以当前不存在混合标签的情况；这只是防御性约定。
3. `PATCH /api/material/prices/<id>`（改供应商）和 `DELETE` **不用拦**——
   已存在的价格记录该能维护和清理，拦住反而没法删掉历史脏数据。
4. 详情响应建议补一个 `can_add_price`（布尔）让前端直接用，
   免得前端自己再推一遍大类逻辑。若不加，我就在前端判 `categories`。

## 前端我这边配合

「添加价格」按钮在无用物料上隐藏，并显示原因提示。
但这只是体验层，**后端必须真拦**——同一套理由，前端隐藏不算门禁。

---

# ✅ 无用物料门禁验收与部署（2026-08-07，`dd9b6fb`）

已部署。1 个文件（`services/product/material_price.py`），MD5 一致，
无迁移，Alembic 仍 `20260807_01`，reload 后 master PID **2106 未变**。

| 验收项 | 结果 |
|---|---|
| 无用物料被拒 | `01.01.AD16002-S1` → `success=False`「无用物料不支持维护价格」（HTTP 400）✅ |
| **拒绝后不留空成本节点** | node 130 → **130**，price 133 → **133** ✅ |
| **基础码未被占用** | `01.01.AD16002` 在 `cost_bom_node` 中**不存在** ✅ —— 这是本次门禁最关键的一条 |
| 请求体伪装绕过 | 带 `is_useless:false` / `category:'material'` 仍被拒 ✅（门禁在方法第一行，先于请求体解析） |
| 正常物料不受影响 | `14ST01010-A01` `categories=['material']` → `can_add_price=True` ✅ |
| 既有价格仍可维护 | 改供应商成功并已还原；`PATCH`/`DELETE` 未被门禁堵死 ✅ |
| 新增 `can_add_price` | 无用物料 `False`、正常物料 `True`，前端已优先使用该字段 ✅ |

测试：本机 `307 tests, failures=0, errors=44`（本机 tmp 权限问题），
`261 + 44 = 305`，与 Codex 报告一致。

实现位置正确：门禁是 `add_price` 的**第一条语句**，先于请求体校验、
先于 `_node()`，因此不存在"先建节点再失败"的窗口。

---

## 🟡 仍开放：`-S1`/`-C1` 被误当版本后缀

Codex 本批未改（等业务确认），我同意暂不动。当前状态：

- 受影响的 **55 个物料全部在 `01 金蝶成品物料`**，已被标为「无用物料」；
- **无用物料现已不能加价格** → 这条路径实际上被门禁堵死了，
  原本的"变体共用价格"风险**不再可达**；
- 线上既有的 58 例 base 码回退全是同零件不同图纸版本，属正确行为。

> 也就是说：无用物料门禁**顺带消除了这个缺陷的实际危害**。
> `_strip_version` 的收紧从"应该修"降级为"可选清理"，
> 优先级可以放到最低。若将来把这两组重新启用，需要重新评估。
