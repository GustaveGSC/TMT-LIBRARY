...
## 未分类物料的处置（用户 2026-08-07 决定）

> 这个未分类里的物料很复杂，我的建议是不去管它，但是也可以进行选择。

**定案：未分类物料不做整理，但长期保持可选。**

- 物料选择器**刻意不按大类限定**，未分类的 3,354 条同样能加入组合；
- 这**不是**「等分组默认大类配好后要收紧」的临时妥协 —— 前端代码注释与
  界面提示都已按既定行为改写，避免后来者误以为这是遗留 TODO 而去加限制；
- 因此 `erp_group_category` 保持 0 行也不影响组合功能可用；
  分组默认大类什么时候配、配多少，与本功能解耦。
