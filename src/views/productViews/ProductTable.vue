...
const allPackagedColumns = [
  ...
  { prop: 'length', label: '长 (cm)', width: 90, align: 'right', sortable: true, formatter: (row) => Number(row.length).toFixed(1) },
  { prop: 'width', label: '宽 (cm)', width: 90, align: 'right', sortable: true, formatter: (row) => Number(row.width).toFixed(1) },
  { prop: 'height', label: '高 (cm)', width: 90, align: 'right', sortable: true, formatter: (row) => Number(row.height).toFixed(1) },
  { prop: 'volume', label: '体积 (m³)', width: 110, align: 'right', sortable: true, formatter: (row) => Number(row.volume).toFixed(3) },
  { prop: 'gross_weight', label: '毛重 (kg)', width: 100, align: 'right', sortable: true, formatter: (row) => Number(row.gross_weight).toFixed(1) },
  { prop: 'net_weight', label: '净重 (kg)', width: 100, align: 'right', sortable: true, formatter: (row) => Number(row.net_weight).toFixed(1) },
  {
    ...
    <el-dialog v-model="showAllPackagedDialog" title="所有产成品数据" width="90%" top="6vh" append-to-body destroy-on-close>
      ...
      >
        ...
        <template #cell-code="{ row }">
          <span style="font-weight:700;color:#2c2420;">{{ row.code }}</span>
        </template>
