// world.json（世界地图 GeoJSON）里全部 217 个区划的英文名 → 中文名。
// 单一数据源：地图渲染（ShippingDashboard.vue）和财务客户映射的国家/地区录入
// （FinanceCustomerMapping.vue）都从这里取数据，避免两处各维护一份、口径不一致。
export const WORLD_NAME_ZH = {
...
// 常见简称/别名 → GeoJSON 英文名。财务人工填写时习惯用简称（如"沙特"而非"沙特阿拉伯"），
// 这里做兼容映射，与下面从 WORLD_NAME_ZH 反查出的完整版合并。
const COUNTRY_ALIASES_ZH = {
  沙特: 'Saudi Arabia', 朝鲜: 'Dem. Rep. Korea',
}

// 完整反查表：中文名（217 个区划的规范名 + 常见别名）→ GeoJSON 英文名
export const ZH_TO_GEOJSON_NAME = Object.fromEntries([
  ...Object.entries(WORLD_NAME_ZH).map(([en, zh]) => [zh, en]),
  ...Object.entries(COUNTRY_ALIASES_ZH),
])

// 供国家/地区录入下拉框使用的规范中文名列表（含常见别名，去重排序）
export const WORLD_COUNTRY_NAMES_ZH = [...new Set([
  ...Object.values(WORLD_NAME_ZH),
  ...Object.keys(COUNTRY_ALIASES_ZH),
