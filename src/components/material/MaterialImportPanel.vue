    ...
    <div v-if="step === 'preview_done'" class="preview-section">
      ...
      <div class="preview-table">
        <div class="pt-head">
          ...
          <div class="pt-col col-name">品名</div>
          <div class="pt-col col-spec">规格</div>
          <div class="pt-col col-status">状态</div>
          <div class="pt-col col-group">品号群组</div>
        ...
        <div v-for="(row, i) in previewSample" :key="i" class="pt-row">
          ...
          <div class="pt-col col-name">{{ row.name }}</div>
          <div class="pt-col col-spec">{{ row.spec || '—' }}</div>
          <div class="pt-col col-status">{{ row.status || '—' }}</div>
    ...
    <div v-if="step === 'done' && importResult" class="result-section">
      ...
      <div class="result-grid">
        ...
        <div class="result-card updated">
          <div class="result-val updated-val">{{ importResult.updated }}</div>
          <div class="result-label">内容更新</div>
        </div>
        <div class="result-card neutral">
          <div class="result-val neutral-val">{{ importResult.unchanged }}</div>
          <div class="result-label">内容未变</div>
        </div>
        <div class="result-card neutral">
...
.material-import::-webkit-scrollbar { width: 4px; }
.material-import::-webkit-scrollbar-track { background: transparent; }
.material-import::-webkit-scrollbar-thumb { background: var(--border); border-radius: 2px; }

.import-title { font-size: 20px; font-weight: 700; color: var(--text-primary); margin-bottom: 4px; }
.import-sub   { font-size: 13px; color: var(--text-secondary); margin-bottom: 24px; }
...
.upload-hint-sub { font-size: 12px; color: var(--text-secondary); }
...
/* ── 预览表格 ─────────────────────────────────── */
.preview-section { margin-top: 28px; max-width: 1000px; }
...
.col-spec   { width: 150px; flex-shrink: 0; }
.col-status { width: 64px;  flex-shrink: 0; }
.col-group  { width: 110px; flex-shrink: 0; }
...
.result-grid { display: flex; gap: 12px; flex-wrap: wrap; }
...
.result-card.updated { border-color: rgba(74,143,192,0.3); background: rgba(74,143,192,0.05); }
