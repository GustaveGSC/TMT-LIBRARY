...
// ── 类型配置 ──────────────────────────────────────
// 这里的 label 必须与后端 TYPE_LABELS 保持一致：筛选标签用本地 label，
// 而表格里的徽章文字取后端返回的 type_label，两边不一致会出现
// 「筛选写物料、徽章写原材料」这种前后矛盾。
const TYPE_OPTIONS = [
  { value: 'finished',  label: '成品',     color: '#c4883a' },
  { value: 'packaged',  label: '产成品',   color: '#4a8fc0' },
  { value: 'semi',      label: '半成品',   color: '#9c6fba' },
  { value: 'material',  label: '原材料',   color: '#6ab47a' },
  { value: 'useless',   label: '无用物料', color: '#8a7a6a' },
]
